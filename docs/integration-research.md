...
따라서 `OfficialCodexAdapter`는 구현하지 않습니다. M3 수동 1회 적용도 현재 조건에서는 진행하지 않습니다. 공식 지원 수단이 새로 공개되면 문서와 실제 설치 버전을 다시 확인한 뒤 별도 승인을 받아 재평가합니다.
...
다음 조건을 모두 충족하기 전에는 M3를 시작하지 않습니다.
